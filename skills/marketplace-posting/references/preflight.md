...
- Platform is selected by `sales-strategy` or the requesting user.
- Price / start price is selected.
- Shipping mode is selected.
- Missing placeholders such as `[BITTE ... EINGEBEN]` are resolved or explicitly accepted by the requesting user.
- Approval is present for this item.

Stop and ask the requesting user if any of these fail.
...
- the requesting user explicitly approves posting this item in the current conversation
...
- Browser context is the persistent verified auction profile, currently the requesting user's Brave setup, or another manually verified context.
